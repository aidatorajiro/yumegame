/*
Begin excepts/modifications from wmctrl version 1.07
...
static Window search_window (Display *disp) {/*{{{*/
    Window activate = 0;
    Window *client_list;
    unsigned long client_list_size;
    int i;
...
/*
End excepts/modifications from wmctrl version 1.07
*/

int main ()
{
    options.verbose = 1;
    options.force_utf8 = 0;
    options.show_class = 1;
    options.show_pid = 0;
    options.show_geometry = 0;
    options.match_by_id = 0;
    options.match_by_cls = 0;
    options.full_window_title_match = 0;
    options.wa_desktop_titles_invalid_utf8 = 0;
    options.param = "";
    options.param_window = "Blender.Blender";
    ...
    XEvent ev;
    unsigned long current_desktop_value;

    for (;;)
    {
        XNextEvent(display, &ev);
        
        if (ev.xproperty.atom == XInternAtom(display, "_NET_CURRENT_DESKTOP", False))
        {
            unsigned long *current_desktop = (unsigned long *)get_property(display, XDefaultRootWindow(display), XA_CARDINAL, "_NET_CURRENT_DESKTOP", NULL);
            if (current_desktop) {
                current_desktop_value = *current_desktop;
                g_free(current_desktop);
                printf("Current workspace id: %ld\n", current_desktop_value);
                Window w = search_window(display);
                printf("Target window id: %ld\n", w);
                if (w) {
                    client_msg(display, w, "_NET_WM_DESKTOP", current_desktop_value, 0, 0, 0, 0);
                }
            }
        }
    }

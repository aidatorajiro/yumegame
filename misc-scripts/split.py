...
def condition_exists(entry):
    return (
        not os.path.exists(os.path.join(basepath, 'movies-left', entry + '.left.mp4')) and 
        not os.path.exists(os.path.join(basepath, 'movies-right', entry + '.right.mp4')) and 
        not os.path.exists(os.path.join(basepath, 'botsu', entry + '.left.mp4')) and 
        not os.path.exists(os.path.join(basepath, 'botsu', entry + '.right.mp4')) and
        not os.path.exists(os.path.join(basepath, 'taihi-left', entry + '.left.mp4')) and 
        not os.path.exists(os.path.join(basepath, 'taihi-right', entry + '.right.mp4'))
    )
...
for entry in os.listdir(os.path.join(basepath, 'movies')):
    if entry.endswith('.mp4'):
        ...
        if condition_exists(entry):
            ...
            subprocess.run([
                'ffmpeg', '-i',
                os.path.join(basepath, 'movies', entry), '-acodec', 'copy', '-acodec', 'none'] +
